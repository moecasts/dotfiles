...
function M.info(buf)
  ...
  Editor[enabled and 'info' or 'warn'](
    table.concat(lines, '\n'),
...
function M.format(opts)
  ...
  if not done and opts and opts.force then
    Editor.warn('No formatter available', { title = 'Editor' })
  end
end
...
function M.health()
  ...
  -- local has_extra = vim.tbl_contains(Config.spec.modules, 'Editor.plugins.extras.lsp.none-ls')
  -- if has_plugin and not has_extra then
  --   Editor.warn({
  --     '`conform.nvim` and `nvim-lint` are now the default formatters and linters in Editor.',
  --     '',
  --     'You can use those plugins together with `none-ls.nvim`,',
  --     'but you need to enable the `Editor.plugins.extras.lsp.none-ls` extra,',
...
function M.snacks_toggle(buf)
  return Snacks.toggle({
    ...
    get = function()
      ...
      return Editor.format.enabled()
    end,
    set = function(state)
      Editor.format.enable(state, buf)
    end,
  })
end
